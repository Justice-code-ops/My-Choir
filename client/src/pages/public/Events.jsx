...
export default function Events() {
  ...
  if (loading) return <div className="text-center py-12 text-gray-600 dark:text-gray-400">Loading events...</div>;
        ...
        {events.length > 0 ? (
          events.map((event) => (
            <div key={event._id} className="bg-white dark:bg-dark-800 rounded-lg shadow overflow-hidden hover:shadow-lg transition">
              {resolveAssetUrl(event.image?.url) && (
                <img src={resolveAssetUrl(event.image.url)} alt={event.title} className="w-full h-48 object-cover" />
              )}
              <div className="p-6">
